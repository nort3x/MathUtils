...
tempT Algebric::MultiDimPoint<T>::MultiDimPoint(int d) {
    mdp.reserve(d);
     dim = d;
}
...
tempT Algebric::MultiDimPoint<T>::MultiDimPoint(const T* p,int dim) {
    mdp.reserve(dim);
    this->dim = dim;
    for(int i =0 ;i<dim;i++){
        setValue(p[i],i);
...
tempT Algebric::MultiDimPoint<T>& Algebric::MultiDimPoint<T>::setValue(T p, int index) {
